...
class DeepgramTranscriber:
    ...
    def extract_audio(self, mp4_file, audio_file):
        ...
        if not os.path.exists(mp4_file):
            raise FileNotFoundError(f"Video file not found: {mp4_file}")
        
        file_size = os.path.getsize(mp4_file)
        if file_size < 100000:  # Less than 100KB is suspicious for a video file
            raise ValueError(f"Video file appears to be incomplete or corrupted. Size: {file_size:,} bytes. Please re-upload the video.")
        
        try:
            video_clip = VideoFileClip(mp4_file)
            if video_clip.duration is None or video_clip.duration <= 0:
                video_clip.close()
                raise ValueError("Video file appears to be corrupted or has no duration")
            ...
            if "moov atom not found" in error_msg or "Invalid data found" in error_msg:
                raise ValueError("Video file is corrupted or incomplete. Please re-upload the video file.")
            else:
                raise e
